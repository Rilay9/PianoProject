...
**Status: P0 through P19 are built (2026-09-06), and so are the rounds after them — P20, the
setup tour; P21 and its tour rounds; P21f, rhythm-only runs, the tempo ladder and the duet row.
There are no phases left.** The app runs end
to end — you can open it, be given a practice session and a daily sight-read, play a piece with
the sheet music following your hands (MIDI, the microphone, the on-screen keys or the clock),
have the run scored and recorded, and see it come back for review; every drill has a screen and
its own advice; every rung says what music would train it and takes a file you found in two
taps; a folder of scores on the phone can be browsed and imported from; books you own on paper
can be practised against; and a progression you pick can be written out or kept in time to play
over. What is on every rung, and how much of everything there is, is `docs/generated/ladder.md`,
which the build regenerates; the record of the work since the phases ended is
...
| Path | What |
|------|------|
| `CLAUDE.md`, `docs/00-invariants.md` | how to work in this tree, and the owner's rules collected — read before any substantial change |
| `docs/00-overview.md` | vision, decisions (D1–D26), assumptions (A1–A8), open questions |
| **`docs/OWNER-GUIDE.md`** | **start here to use the app**: install it, connect the piano, import your own scores, back up your history |
| `docs/decisions/` | one file per decision taken mid-phase, dated, with the reasoning; `docs/decisions/README.md` indexes them |
| `docs/08-score-render-states.md`, `docs/08-test-map.md` | the score screen's state space and its walk against the code; which test proves which state machine, and an index of every spec |
| `docs/handoff-2026-09-09.md`, `docs/night-plan-2026-09-10.md` | the running record since the phases ended, and one night's plan |
| `docs/generated/ladder.md` | generated by the build: what is on every rung, and the counts |
...
| `docs/06-build-plan.md` | phases P0–P21e, model assignment, acceptance criteria, test strategy |
| `docs/07-midi-hp130-notes.md` | short MIDI notes and the deferred checklist |
| `prompts/` | paste-ready prompts per phase (P0–P21e, with P3b, P5b, the common header, the replan brief and PR-review) |
