---
title: "Playing for people who are singing"
stage: 3
unit: "holiday.3.1"
readingTime: 3
---

The carol rung below this one is about playing the tunes. This one is about the
job you will actually be asked to do with them, which is different: a room full
of people who half remember the words, singing at whatever speed they decide,
while you hold it together.

**The key is theirs, not yours.** A carol written in a comfortable piano key is
often too high to sing. Untrained voices mostly live between about A below
middle C and D above it, and if the tune goes higher than that a room will
quietly stop. Being able to move a tune down a step or two is worth more here
than anything about technique.

**Which is why the exercises are one cadence in three keys.** C, G and F, the
same four chords each time. Transposing is not a trick; it is knowing a
progression by its numbers rather than by its letters, so the shapes come to
hand in a key you did not plan for. Do the same cadence in all three and the
third one will take a fraction of the time the first did.

**Play the tune, not an arrangement.** A singing room needs the melody clearly
and the harmony underneath, and it needs both to be loud enough and steady
enough to follow. Ornament nothing. If you drop a beat they all drop a beat.

**Give them the last line before you start.** Play the final phrase once as an
introduction. It tells everybody the key, the speed and — because it is the bit
they know best — when to come in.

**Repertoire.** Four options, all with the chords printed. *Jingle Bells* is in
G with block chords already written, so it is the one to read first. *Joy to the
World* and *The First Noel* are in D and are lead sheets: tune on top, symbols
above. *Hark! The Herald Angels Sing* has the most chord changes of the four and
is the one to take slowest.

**Tools for this rung.** *Free play* names the chord you are holding, which is
the quickest way to check a transposition — play the shape you think is right
and see whether the app agrees it is a G.

**Common mistake.** Following the singers. Somebody always drags, and if you
follow them the whole room slows down until the carol stops. Keep the pulse and
let them come back to you; they will.

**How you'll know you've got it.** You can play one of these in its written key
and then again a step lower without stopping to work it out, and hold a steady
tempo through both.
