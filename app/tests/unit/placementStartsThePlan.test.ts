...
const passes = (...ids: string[]): PassRecord[] => ids.map((itemId) => ({ itemId, passed: true }));
...
describe('with a placement', () => {
  it('starts at the placed unit rather than at the beginning', () => {
    expect(nextRecommended(curriculum(), [], [], { startAt: '3.2' })?.lesson.id).toBe('3.2');
  });
  ...
  it('takes a rung id too, because the lesson page’s *Start here* writes one', () => {
    ...
    expect(nextRecommended(curriculum(), [], [], { startAt: 'jazz.3' })?.lesson.id).toBe('jazz.3');
  });

  it('moves on from the placed rung once it is complete', () => {
    const done = passes('exercise.3.2', 'song.3.2');
  ...
  it('comes back to the rungs behind it when there is nothing left in front', () => {
    const done = passes('exercise.3.2', 'song.3.2', 'exercise.jazz.3', 'song.jazz.3');
  ...
  it('ignores a placement at a unit the curriculum does not have', () => {
    ...
    expect(nextRecommended(curriculum(), [], [], { startAt: 'blues.4' })?.lesson.id).toBe('1.1');
  });

  it('is off when the placement is empty, which is a fresh plan row', () => {
    expect(nextRecommended(curriculum(), [], [], { startAt: '' })?.lesson.id).toBe('1.1');
  });
