 ...
 * row (`SessionRow.evidence`), under the row's own `definitions`. That is a
 * cache of a derived value (design §5 allows one), not a stored state: this
 * module reads it back and derives the state from it every time.
 *
 * **What contributes.** A row whose `definitions` is the version in force and
 * which carries stored evidence. A row with none (written before C4, or by a
 * screen that had no model), or one stamped with another version, contributes
 * nothing: it is not re-derived here, because re-deriving needs the played
 * model, which a row does not carry. Refusals are kept on the row for the sheet
 * and the record; only evidence reaches the ladder.
 *
 ...
 */
import { OBSERVATION_DEFINITIONS, type SessionRow } from '../data/db';
import type { Skill, SkillKind } from '../demands/vocabulary';
import { isRefusal, type Evidence } from './evidence';
 ...
 * row is stamped with the definitions in force; nothing otherwise.
 */
export function storedEvidence(row: SessionRow): Evidence[] {
  if (row.definitions !== OBSERVATION_DEFINITIONS || !Array.isArray(row.evidence)) return [];
  const out: Evidence[] = [];
