...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'Every card whose answer is a chord shows you that chord on a staff the moment it is judged, right or wrong, costing nothing — the chord and inversion cards, the seventh voicings, the roman numerals, and the ear drills that play you a progression, which come up one chord to the bar with the numeral over it so the numerals and the notes line up. The card is held while you read it, and a tap moves on. Never before you answer: a chord with its notes under it is a card you cannot get wrong. The pedal and backing-track cards, where the notes are not what is being judged, print theirs from the start.',
      ...
      'Three chips on the Simon card say how much help it gives: the keys lit and named as the chain plays, with each note landing on a staff on the card as it sounds where the screen is tall enough to hold one, the keys lit on a replay after a wrong note — which then asks you for the same chain again instead of ending the game — or sound alone. Each drill remembers the one you chose for it, and none of them discounts the score: the chain is gone by the time it is your turn either way.',
