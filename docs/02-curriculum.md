...
- **Level versus rung (2026-09-06).** A catalog item's `level` is one global difficulty, used
  for swaps, sorting and the session builder. A track's *rung* (Stage 5, 6, 7… in Part D) is
  an order within that track. The two disagree on purpose where the repertoire forces it —
  the easiest complete Joplin rag is a Grade 6 piece, so the ragtime Stage 6 rung holds
  level-7 songs. Each lesson declares a `levelBand: [min, max]` that its song options must
  fall inside (`validate.py` checks), and the lesson page prints it. A level that was
  estimated rather than judged carries `levelSource: "estimated"` and is shown as `≈ 7.1`;
  the owner can re-level any item from its detail sheet. See the replan decision, §1.4 and
  §1.7.
- **Finder (from P15).** Every lesson and every concept carries a `finder` block — the skill
  in words, the level in words, constraints, what to avoid, named examples (bundled or
  wanted) — from which the build generates a search query and a chatbot prompt the owner can
  paste to go and find more music himself, including music still in copyright. A piece the
  ladder wanted and no source had is a finder example, not a dead option.
- **Source tags** for repertoire: `[AUTH]` = builders author it in ABC/tinyNotation (short
  public-domain melodies, easy arrangements — **including a simplified arrangement of a
  public-domain piece when a ladder asks for an "easy arr." and no source has one**);
  `[MT]` = MuseTrainer public-domain MusicXML library (github.com/musetrainer/library);
  `[KERN]` = Humdrum `**kern` repositories by Craig Sapp on GitHub (`--allow-nc` only);
  `[NIFC]` = the Fryderyk Chopin Institute's first editions (CC BY); `[PDMX]` = scores
  **quarried** from the PDMX dataset on the owner's machine and committed under
  `content/scores/pdmx/` — a few hundred selected files, never the dataset (see
  `03` §2 and the replan decision §2); `[MUTO]` = Mutopia Project (LilyPond source →
  convert); `[IMSLP]` = IMSLP MusicXML/PDF (PD scans; use only if a MusicXML exists);
...
**Source note (2026-09-06):** all the Chopin above is now in the library from `pl-wnifc/humdrum-chopin-first-editions` — the Fryderyk Chopin Institute's encodings of the first editions, CC BY 4.0, tagged `[NIFC]` in `03` §2. That replaces the `[KERN]` tag on every Chopin entry in this ladder: the two `craigsapp` Chopin repositories state no licence and are permanently excluded. 183 solo-piano works were surveyed and 169 imported: Étude Op. 25 no. 7 cannot be converted at all, and thirteen more convert cleanly but OpenSheetMusicDisplay's engraver refuses to draw them. Stages 6-9 of this track and of D5 now offer 8-23 options per rung.
...
- **Levels come from one table**, `LEVELS` in the generator, keyed by family and parameters
  and derived from the table above (P12a). Until P12a every scale variant is levelled `4.1`,
  which is why 225 exercises sit at level 4 and none above 5.
- **Hanon 21–60 are not encoded.** No reachable public-domain edition of them exists and
  encoding sixty exercises from memory is what P4 refused to do for 1–20. The skills they
  train — repeated notes, double thirds and sixths, trills, tremolo, rotation — are generated
  as named families instead ("Repeated notes 4-to-a-note in C", not "Hanon 44"). The
  "Studies" column's Czerny, Bertini, Heller and Cramer entries are repertoire to be quarried
  or found, not generated.
- **Families added in P12a:** scales 3–4 octaves, in 3rds and 6ths, octave scales, chromatic
  2 octaves, maj7/min7/half-diminished arpeggios, broken 7ths, the Hanon-style cells above,
  articulation pairs (staccato/legato, scored by note length), hand independence 2:1 / 3:1 /
  2-against-3, dynamics shaping (crescendo slope), voicing (top note louder), ties across
  the bar and 16th-level syncopation, 5/4 and 7/8, held-melody and half-pedal rows.
- **Families added in P12b:** seventh voicings (close, shell, rootless A/B), ii–V–I and the
  four-chord loop in twelve keys, slash-chord bass lines, walking bass, comping rhythms,
  stride, turnarounds, tritone substitutions, quartal and sus/add9 voicings, boogie
  variants; drills for modes, secondary dominants, modulation, harmonic dictation as
