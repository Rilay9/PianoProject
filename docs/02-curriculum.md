...
**Source note (2026-09-06):** all the Chopin above is now in the library from `pl-wnifc/humdrum-chopin-first-editions` — the Fryderyk Chopin Institute's encodings of the first editions, CC BY 4.0, tagged `[NIFC]` in `03` §2. That replaces the `[KERN]` tag on every Chopin entry in this ladder: the two `craigsapp` Chopin repositories state no licence and are permanently excluded. 183 solo-piano works were surveyed and 182 imported; Étude Op. 25 no. 7 is the one that could not be converted. Stages 6-9 of this track and of D5 now offer 8-23 options per rung.
