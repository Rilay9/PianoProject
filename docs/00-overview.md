...
| D20 | **Offline-first: everything is downloaded and runs locally** (owner, 2026-09-05). The app precaches its shell and the *entire* content library on first launch and never needs the network again. The only online moments are the first launch, an update check (which fails silently offline and can be turned off), and the teaching-video links, which are external by design and labelled as such. This settles the `00` D19 origin question in favour of **full offline precache**. | The owner has plenty of storage and asked to minimise what needs the internet. The whole payload is small: measured 2026-09-05, content is **6.0 MB** (2.7 MB scores, 2.6 MB soundfont, 0.5 MB catalog, 0.2 MB lessons) and the built app 7.6 MB — two orders of magnitude under the 60 MB precache budget in `01` §6, even after P5b and P10 expand the library. |
