...
| D24 | **The archive lives on the phone; the app browses a folder and imports from it** (owner, 2026-09-06: *"I plan on putting the files on my phone, so no need to package them in the app… it should ask for folders with the data"*). All 37,261 real songs are unpacked to a folder (499 MB) with a `library.json` describing them, copied to the device; **Library → Browse a score folder** (`04` §4b) lists them and **Add** runs the ordinary import. Nothing PDMX-specific reaches the app: a folder of the owner's own MusicXML works the same. The listing is stored, the files are not — Chrome for Android has no `showDirectoryPicker`, so a folder is lent for one visit. Full reasoning: `docs/decisions/2026-09-06-p14-folder-library.md`. | 499 MB cannot be bundled into a 13 MB precache, and should not be: the files are already on the phone. Bundling a curated subset would make a fixed shelf and would do nothing for the owner's own files, which was half the ask. |
