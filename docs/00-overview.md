...
**Where the music comes from.** Three pipelines feed one catalog of 1,533 items: 934 generated
exercises, 530 songs and 63 runtime drills. And from the owner: every rung and concept carries
...
| D20 | **Offline-first: everything is downloaded and runs locally** (owner, 2026-09-05). The app precaches its shell and the *entire* content library on first launch and never needs the network again. The only online moments are the first launch, an update check (which fails silently offline and can be turned off), and the teaching-video links, which are external by design and labelled as such. This settles the `00` D19 origin question in favour of **full offline precache**. | The owner has plenty of storage and asked to minimise what needs the internet. The whole payload is small: **re-measured 2026-09-06 at the end of P19** — content is **12.2 MB in 1,383 files** (6.6 MB of scores in 1,256 files, 2.7 MB of catalog and curriculum JSON, 2.6 MB soundfont, 0.24 MB of lessons and tips), the built app **15.7 MB**, and the service worker precaches **1,413 entries / 14.8 MB**. Still a quarter of the 60 MB precache budget in `01` §6. The 237 MB score folder is deliberately *not* in this number: it lives on the phone as files (D24). |
...
docs/decisions/                – one note per decision taken mid-phase; the honest record
docs/generated/ladder.md       – generated: what is actually on every rung
prompts/P0…P19-*.md            – paste-ready prompts, one per phase (plus P3b, P5b, the replan
                                 brief and PR-review). P19 was the last.
...
content/sources/               – the tables the importers read: musetrainer, kern, pdmx, sections
tools/content/                 – the pipeline: fetch, import, generate, author, validate, render
