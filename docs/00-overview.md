...
This repository began as a *planning and instruction package*, written by an AI session at
the request of the project owner (Yali). The code was then built by AI sessions following the
numbered prompts in `prompts/`. Every doc in `docs/` is referenced by those prompts; the prompts
are self-contained but assume the builder can read this repository. (No AI model is named
anywhere in this repository — `00-invariants.md` §4.)
...
*Rewritten 2026-09-06 from what was actually built (`docs/decisions/2026-09-06-once-over.md`
§1). The paragraph that stood here described the plan on day one and had stopped describing
the app; the decisions below are the record of how it changed and are never rewritten.*

A piano-teaching app for one person, one phone and one piano: the owner, a Samsung Galaxy S25,
and a Roland HP-130 over a USB MIDI cable. It is a web app (Vite, TypeScript, no framework)
installed from the owner's own laptop over HTTPS (D25) and wrapped as a Trusted Web Activity
APK (D19) so Android treats it as a real app while Chrome keeps supplying Web MIDI. Everything
it needs is on the phone after the first launch. Nothing has an account, a server or a network
dependency except the teaching-video links, which say so before you tap them.

**At the piano.** It draws real sheet music — grand staff, a window of a few bars — and moves
...
branches. Beautiful is not one of them."* Its content was never a branch either — the lesson's own
first line was "This is not a ladder; it is a shelf" — so it is now a second lesson on the
classical stage-4 rung, `classical.4.shelf`, and the repertoire it collects is tagged `classical`
like the rest of it.

**Where the music comes from.** Three pipelines feed one catalog — generated exercises (the
largest share), songs, and a handful of runtime drills; the counts by type are on the
Diagnostics screen (`04` §7b) and in `docs/generated/ladder.md`. And from the owner: every rung and concept carries
a *finder* that says what music would train it; a file he finds reaches the right rung in two
taps from Android's share sheet; a folder of scores on the phone is browsed and imported from
(D24); books he owns on paper are registered on a shelf and practised against.

**What keeps it honest.** `validate.py` refuses a build with a thin rung, an orphan exercise, a
stale ladder report, a missing finder or tips file, a section past the last bar, or an undefined
track. The render check opens every score in a real Chromium. The licence gate is code: one
table builds two catalogs, the owner's `--personal` one and the redistributable one.
...
| D13 | **Curriculum as data** (`content/curriculum/*.json`), not code. Lessons, options, and prerequisites are JSON validated by a schema. | Lets a content-authoring session add content without touching the engine; lets the owner edit the plan by hand. |
...
| D18 | **Copyrighted band repertoire (Avenged Sevenfold, Linkin Park, Sleep Token, film/game/modern pop) is taught as *technique* on public-domain material plus an import path for the actual songs.** A "Rock & metal piano" module and a collection of the pieces people learn the piano for are part of the curriculum. **Amended 2026-09-12:** the collection is a lesson on the classical stage-4 rung, not a track of its own — the owner struck "Beautiful" as a branch because the branches are genres and that was a taste. | Legal; and the techniques (minor ostinatos, octave bass, sus chords, ambient pedal textures, reductions of band arrangements) are exactly what those songs need. |
...
| D20 | **Offline-first: everything is downloaded and runs locally** (owner, 2026-09-05). The app precaches its shell and the *entire* content library on first launch and never needs the network again. The only online moments are the first launch, an update check (which fails silently offline and can be turned off), and the teaching-video links, which are external by design and labelled as such. This settles the `00` D19 origin question in favour of **full offline precache**. | The owner has plenty of storage and asked to minimise what needs the internet. The whole payload is small: the content directory is a few thousand small files, most of them scores, plus the soundfont, and the service worker precaches all of it well inside the 60 MB precache budget in `01` §6 — around a third of it as of the 2026-09-15 quarry re-run, up from a quarter at the end of P19. The live figures are what `npm run pwa:audit` (from `app/`) prints for a build and what Settings → Diagnostics shows on the phone; they are not written here because they move with every content change. The score folder on the phone, a few hundred MB, is deliberately *not* in this number: it lives on the phone as files (D24). |
...
| D24 | **The archive lives on the phone; the app browses a folder and imports from it** (owner, 2026-09-06: *"I plan on putting the files on my phone, so no need to package them in the app… it should ask for folders with the data"*). All 37,261 real songs are unpacked to a folder (237 MB) with a `library.json` describing them, copied to the device; **Library → Browse a score folder** (`04` §4b) lists them and **Add** runs the ordinary import. Nothing PDMX-specific reaches the app: a folder of the owner's own MusicXML works the same. The listing is stored, the files are not. **Corrected 2026-09-06 (P19), and again 2026-09-12:** MDN lists `showDirectoryPicker` from Chrome for Android 132 and an installed PWA keeps the grant, so **Remember the score folder** now defaults **on** and the app re-picks the folder only when the stored handle fails; what still needs the phone is whether the S25 keeps the grant across a relaunch. The current rules are in `04` §4b, which is the place they are kept. Full reasoning: `docs/decisions/2026-09-06-p14-folder-library.md`. | 237 MB cannot be bundled into a 13 MB precache, and should not be: the files are already on the phone. Bundling a curated subset would make a fixed shelf and would do nothing for the owner's own files, which was half the ask. |
| D25 | **The app is served from the owner's laptop over the house Wi-Fi, only while installing or updating; "Add to Home screen" is the everyday route and the APK is optional** (owner, 2026-09-06: *"do option 1, but allow and give instructions for #3"*). A TWA and an installed PWA both load from an HTTPS address once and then run from the service worker; the address can therefore be `packaging/serve-lan.py` on the laptop with a certificate from **mkcert** whose root is installed on the phone, so nothing about the personal build is ever reachable from outside the house. Supersedes the "static host" half of D19 and the last of D9: there is no public deploy, `pages.yml` goes when the repository is private, and CI keeps running the tests. The APK path (Bubblewrap, Digital Asset Links at the laptop's origin root, port 443) is kept and documented, with the caveat that Android's link verification against a bare IP address has not been tried. | The owner asked why he needed a host at all. He does not need a *third party's*: he needs an HTTPS origin for one launch, and the laptop is one. A public URL of a build that carries NC editions and in-copyright transcriptions was the wrong shape for a personal app. |
| D26 | **Four rules every screen obeys** (2026-09-07): the subject first, two reading distances, weight by frequency, nothing dead. Written out in `04` §0; the reasoning, the nine questions they answer and the per-screen consequences are in `docs/decisions/2026-09-07-ux-decisions.md`. | The app was made to photograph itself and the pictures were read. Twenty small layout fixes were about to be made one at a time, which is how an app comes to look like twenty different opinions. Four rules settle nine questions and can be checked by the tour. |
| D14 | **Model assignment:** the stronger model for architecture, algorithms and debugging on device; the faster model for well-specified UI, data entry, tests, docs, and content authoring from templates. Neither is named, here or anywhere (`00-invariants.md` §4). | Spelled out per task in `06-build-plan.md`. |
...
| A2 | ~~Classical + chords first~~ **Answered:** classical first and "all beautiful songs"; rock/metal piano (Avenged Sevenfold, Linkin Park, Sleep Token) via import + a technique module; **blues and jazz to play with a friend**; pop/chords to play along for himself. **Corrected 2026-09-06:** the tracks the data switches on for a new learner are `core`, `classical`, `chords-pop`, `theory-ear`, `technique` and `practice` (`content/curriculum/00-tracks.json`); blues, jazz, ragtime, rock-metal and jam start off and are one tap away in Plan. That follows A1 — nothing is assumed — rather than this row, which described the owner's tastes rather than a default. Whether blues and jazz should start on is a question for him and one flag in that file. | — |
...
```
README.md                      – index, status, how to build and install
CLAUDE.md                      – orientation for an agent working in this tree; points at the invariants
docs/00-invariants.md          – the owner's rules, collected; read before any substantial work
docs/00-overview.md            – this file
docs/OWNER-GUIDE.md            – the owner's manual: install, connect the piano, own scores, backups
...
content/catalog.static.json    – hand-written catalog rows (runtime drills, import placeholders)
content/sources/               – the tables the importers read: musetrainer, kern, pdmx, pdmx-wants,
                                 composers, hanon-mutopia, level-model, sections, and the rest
content/curriculum/, lessons/, tips/, scores/ – the source content the build copies through
tools/content/                 – the pipeline: fetch, import, generate, author, validate, render (03 §3)
tools/contact_sheet.py         – tiles the corpus run's pictures into one sheet
tools/midi-cleanup/            – a small MIDI utility, outside the pipeline
.github/workflows/             – ci.yml, pages.yml, render-full.yml (the on-demand full render check)
app/                           – the app itself (see app/README.md for the commands)
```
