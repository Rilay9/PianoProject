...
1. `extract.py --from-index --shard` unpacks every indexed *song* — 37,261 files, 499 MB, in
...
- **Bundling a subset into the app.** Was offered — 2,725 well-rated scores is 37 MB, which is
  shippable. Refused, and rightly: it makes a fixed shelf, changing it means a rebuild, and it
  does nothing for the owner's *own* files, which was the other half of the ask.
...
- **Precaching the whole library.** 499 MB against a 13 MB precache. Not arguable.
...
`tools/content/pdmx/select.py` shares a name with the standard library's `select`. That was
...
Every entry point now drops its own directory from `sys.path` before importing anything that
could reach for it. `TestEntryPointsRunAsScripts` runs each one and checks the standard
library's `select` is still the real one.

Renaming `select.py` to something that is not a standard library module would remove the whole
class of bug, and is a follow-up rather than part of this change.
