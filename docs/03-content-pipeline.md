...
| `[KERN]` | **GitHub** `craigsapp/*` Humdrum repos: `mozart-piano-sonatas`, `beethoven-piano-sonatas`, `chopin-preludes`, `chopin-mazurkas`, `scarlatti-keyboard-sonatas`, `joplin`, `bach-370-chorales`, `haydn-piano-sonatas` (verify names) | `**kern` | clone; `music21.converter.parse(file.krn)` → `write('musicxml')` | Scholarly encodings (CC BY-SA / CC0 — check each repo's README/LICENSE). Bach Inventions/WTC also exist as kern on kern.humdrum.org and in `craigsapp/bach-wtc`-style repos — search GitHub for `bach-inventions humdrum` and record what is found. |
