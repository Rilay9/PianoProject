...
- `drills-review.spec.ts` — a miss pauses, going over the missed ones, Simon; observed with a `MutationObserver` (`04` §5c).
...
- `simonDrill.test.ts` — Simon: the chain grows, breaks, is scored, and the two catalog items played.
