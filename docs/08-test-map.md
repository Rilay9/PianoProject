...
- `drills-review.spec.ts` — a miss pauses, going over the missed ones, Simon and its three levels of help; observed with a `MutationObserver` and, for the lit chain, a sampler (`04` §5c, §5c-2).
...
- `answerSheet.test.ts` — the staff behind *Show me*: the answer as notation with a key signature that fits.
...
- `simonDrill.test.ts` — Simon: the chain grows, breaks, is scored, the two catalog items played, the three levels of help and the missed chain that comes back, and `simonForStage` checked against where the curriculum puts them.
