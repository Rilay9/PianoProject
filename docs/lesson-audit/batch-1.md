...
- [ ] **THEORY** `content/lessons/2.5.md:18` — "*Ode to Joy (full theme)* uses one in bar 12, where the tune drops to the G below middle C."
  ...
  - T22 (second reader, 2026-09-22): **RIGHT** on the lesson, and the fingering is settled against the file rather than left open. Bar 12 of `song.classical.ode-to-joy.full` is `C4(1) D4(2) G3(5)` with all three notes on **staff 1, voice 1** (the left hand's own `G2(5)` is a separate note on staff 2). A right-hand 5 on a note a fourth below a right-hand 1 is not a fingering any hand plays without crossing itself, so the marking is a fault in the score file and not a fingering to teach. The lesson's sentence — a shift, in bar 12, to the G below middle C — is exactly what the file prints, so **no lesson edit**. The bad `<fingering>` is a follow-up for whoever owns `score_checks.py`, which has no fingering rule.
