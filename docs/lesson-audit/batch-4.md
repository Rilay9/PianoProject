...
- [ ] **HISTORY** `content/lessons/latin.md:42` — "*La Cumparsita* (1916)"
  ...
  - T22 (second reader, 2026-09-22): **WRONG to print, and removed.** A second search shaped differently from Reader 1's — not the title and not `editionNotes` alone but a four-digit-year regex over *every field* of *every* catalog row whose id holds `cumparsita`, which is three rows — returns **no year anywhere**. The same search over the rung's other dated title returns one: *Tico-Tico*'s `editionNotes` end “pd — Tico-Tico no Fuba (Abreu 1917)”, so the (1917) the lesson prints **is** carried and stays. Reader 1 was right that the year is in the catalog and wrong about where: the title is “Tico-Tico no Fubá” and carries no year. “(1916)” is dropped from `latin.md`. Row added.
...
- [ ] **HISTORY** `content/lessons/classical.6.md:26` — "Chopin's own description was that the left hand keeps time while the right hand is free"
  ...
  - T22 (second reader, 2026-09-22): **rewritten**, because nothing here can decide it and a second reader with no source is not a decision. The attribution is dropped and the image kept, which is Reader 1's own smallest safe change: “Chopin's own description was that…” is now “The old description of it is that…”. The sentence no longer asserts who said it, so nothing is left disputed. No test row: there is no file for one to read.
...
- [ ] **HISTORY** `content/lessons/ragtime.6.md:75` — "(1902)", "(1901)", "(1901)"; "a rag two-step"; "its name is a joke"
  ...
  - T22 (second reader, 2026-09-22): **the three dates are RIGHT**; the subtitle and the joke are cut. The years are in the repository and three readers in a row missed them because they searched the *built* catalog: `content/sources/kern.json` carries a `publishedYear` for **47** Joplin pieces, including *The Entertainer* 1902, *Peacherine Rag* 1901 and *The Easy Winners* 1901 — exactly the three this lesson prints. The field never reaches the catalog (`catalog.schema.json` has no `publishedYear` and no built row carries one), which is why it looked unsourced. “a rag two-step” and “its name is a joke” have no such source and are removed. Row added; carrying `publishedYear` through the build is a follow-up.
...
- [ ] **HISTORY** `content/lessons/blues.6.md:28` — "Jimmy Yancey's own left hand was a dotted habanera figure ... and he ended almost everything in E flat"
  ...
  - T22 (second reader, 2026-09-22): **rewritten.** Reader 1's search was re-run and a second one added over `content/sources/*.json` as well as the built catalog; `yancey` returns nothing in either, so there is no Yancey score here and no way to check either claim. Both stop being assertions: “he ended almost everything in E flat” is gone, and the dotted figure is now written as *how his left hand is usually described*, with the lesson saying in the same breath that there is no Yancey here to check it against. No test row — the point of the edit is that there is nothing to read.
