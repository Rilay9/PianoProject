...
| **C5** | One path from evidence to rung state: requirements as predicates, `rungState`, a run counts toward the rung it was judged by, sight-reading without piece semantics, the recompute job, the other two learners; the old pass-and-count machinery retired | build, browser | done 2026-09-26 (Entry 79); the C5 stop |
| **C6** | The other slots read the evidence (technique, review, new, repertoire), the fallback ladder as stated claims, alternatives that share a skill, reasons from evidence; the reader-policy and evidence-hygiene rows C5 left follow as C6b | build, browser | draft; on the reviewer's C5 verdict |
| **C7** | The Skills store retired: one skill state (the ladder's), rusty as not shown lately, unmeasured concepts say so, Progress shows competence | build, browser | draft; after C6 |
