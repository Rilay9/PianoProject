...
## 8. After the fix-forward (2026-09-26)

Both findings are fixed, each seen red first (Entry 79, item 8). `done` now reads only the rows the judging rung judged, like the other three kinds, with four adversarial cases (finished from nowhere, or from another rung listing the item: not met; from the rung: met; with items missed or below the rung's standard: not). Scoping it exposed a consumer it would have broken: the tour leaves the drill screen on every step and returned with no rung, so 0.3 could never have been met through the tour it offers; the drill screen now keeps the rung that began the tour and judges the finished tour by it. The evidence job walks the sessions store rather than the current catalog, takes every run with a missing or old stamp whose item declares skills or is gone from the catalog and bore evidence, and reports an item-gone row as kept out; tested through `runEvidenceJob`, and seen on the storage report. The `phraseMatches` limitation is untouched and stays at P1 (L80). The chain over the fixed tree and CI are recorded when they land.
