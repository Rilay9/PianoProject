...
| §4.3 keys: the guide, the finger numbers, the flash as three settings | the guide was always on and never two notes ahead in Wait; no finger numbers on the keys; the flash could not be turned off | **done** (2026-09-09) — `keysGuide`, `keysFingerNumbers`, `keysFlash`; the session's `StripOptions`; the gallery's §9.11 knows two-ahead is a setting |
| §4.3 keys: a verdict is a flash, not a state | every judged key stayed green or red for the run — the owner: "the keyboard turning all red and staying that way is bad" | **done** (2026-09-09) — a verdict lands on its key for 900 ms (`KEY_FLASH_MS`) and the key goes back to blue, paler blue or plain; the notation and the summary keep the record |
...
| the state gallery, read cell by cell (39) | the builder's gallery drove a hook that did not exist, so every "notes in" cell showed the idle state under a running caption; corrected, it showed: the mode select clipping "Wait for me" at 412 px; a bar of live controls on a screen with no score, and `display: flex` beating `[hidden]` on it; the count-in overlay centred on the second system's notes once the slots packed; the bar count squeezing the title to "Hot Cr…" beside a status line; stale note classes on a buffer the rotation hid; and in the harness, a wrong note two octaves off the strip and Free undrivable | **done** — short labels to 440 px; the bar hidden and `[hidden]` honoured; the count-in low on the stage; the count gives way to the status upright; classes stripped on a change of arrangement; the harness plays a wrong key the strip can show and drives Free by the step's own notes |
...
| the corpus: ten pieces on four form factors, every step measured (`tests/tour/corpus.spec.ts`) | one song was the proof; the invariants had never been run on a pickup, a repeat, 6/8, chord symbols, a key signature or the 780-bar piece | **done** — 40 legs, a log a leg, three pictures a leg on contact sheets (`tools/contact_sheet.py`); what it found is the rows below |
| §10 a pickup | every range on a pickup piece was drawn one bar late: the engraver counts its draw range from the pickup's own number 0, so the slot the plan called bar 1 held bar 2, the cursor's notes sat in a slot the settle then re-drew with bar 3, and nothing was ever coloured — on *Happy Birthday* and both *Greensleeves*, on every form factor, since the slots were built | **done** — the model says `pickup`, `OsmdView.setRange` passes indexes when the sheet has one, and a block of bars is the pickup with the bars it leads into then tiles from bar 1, so no range starts on the one bar the engraver cannot start on (`slots.rangeAt`, `windowFor`) |
| §6.1 colouring on a repeat | on the first pass of the Minuet in G nothing was coloured: a repeat's second pass has ids of its own for the same printed notes, one `<g>` answered to two, and the per-id toggle let the pass with no state undo the one with | **done** — `setNoteStates` resolves per element, the cursor first |
| §9.12 the cursor's notes carry the colour | neither the sequence nor the corpus checked it; the two faults above were invisible to every picture read so far because a band over a white note looks like a band over a note | **done** — every step now asserts the notes coloured current are the notes the run waits for, and that the cursor's note is in the run's bar |
| §5.1 the next bar at a repeat | the check looked for the bar printed after, so the Minuet failed at bar 32 when the next bar played was bar 17 | **done** — the hook says `nextBar` in playing order; the corpus and the sequence check that bar, and wait up to a second for a heavy piece's read-ahead, recording the lag |
| §3.3 the stave on a piece the probe cannot measure | the Scherzo's stave moved 37 px between a window with a dynamic over it and one without: with no measurement the placement anchored on the ink | **done** — the most any window has had above its stave is held like the sizes, and the stave is placed by it |
| §3.4 the opening tempo word | *Allegro* over the Minuet's first bar was cut off at the top of the slot: the fit holds every stave where the tallest thing above any stave needs it, and a word over the first bar alone is not that | **done** — off on the score screen with the metronome mark, for the same reason; the bar says the tempo |
...
| §3.2 a narrow system sits hard left | `centredInset` refuses to centre any window `mayStretch` permitted, reasoning that music allowed to fill the width already sits where it should. Permission is not the same as having filled it: OSMD may decline the stretch and engrave the bar at its natural width, which is what the owner asked for, and the leftover then goes entirely to the right margin | **open** — dropping the guard does centre it, but it also changes the *engraving* rather than only the position: the tuplet fixture upright went from 324 px of ink to 209 px, which is the stretch `mayStretch`'s own comment calls "the whole point" for a treble-only tuplet piece on a phone. Ten `score.spec` snapshots move. Needs the page-width path understood first |
